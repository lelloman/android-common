...
class TimeTest {
    ...
    fun `throws exception if hour is less than 0`() {
        assertThatThrownBy {
            TimeImpl(hour = -1 * abs(random.nextInt()), minute = 10)
        }.isInstanceOf(IllegalArgumentException::class.java)
    }
    ...
    fun `throws exception if hour is greater than 24`() {
        assertThatThrownBy {
            TimeImpl(hour = 24 + abs(random.nextInt()), minute = 59)
        }.isInstanceOf(IllegalArgumentException::class.java)
    }
    ...
    fun `throws exception if minute is less than 0`() {
        assertThatThrownBy {
            TimeImpl(hour = 3, minute = -1 * abs(random.nextInt()))
        }.isInstanceOf(IllegalArgumentException::class.java)
    }
    ...
    fun `throws exception if minuter is greater than 60`() {
        assertThatThrownBy {
            TimeImpl(hour = 13, minute = 60 + abs(random.nextInt()))
        }.isInstanceOf(IllegalArgumentException::class.java)
    }
    ...
    fun `does not throw exception for valid arguments`() {
        for (hour in 0 until 24) {
            for (minute in 0 until 60) {
                ...
                val tested = TimeImpl(hour = hour, minute = minute)
